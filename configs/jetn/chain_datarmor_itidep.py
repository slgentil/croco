...
ptb = np.arange(-1., 1.1, .5) # range of dcos values
nmode = 3 # mode perturbed: 2 = baroclinic mode 1 / !! make sure nmode_itide is also changed

# check number of arguments

if  len(sys.argv) < 7 :
    print('--------------------------------------------------------------------------------')
    print('[syntaxe] : chain_datarmor.py workdir nbchain elaptim resolution jobname restart_dir') 
    print('--------------------------------------------------------------------------------')
...
workdir_base=sys.argv[1]
nbchain=int(sys.argv[2])
elaptim=sys.argv[3]
resolution=int(sys.argv[4])
jobname_base=sys.argv[5]
restart=1
restart_dir=sys.argv[6]
...
for i, p in enumerate(ptb):

    workdir = workdir_base+'_p%d'%i
    jobname = jobname_base+'%d'%i
    ...
    for t in range(1,nbchain+1):
        ...
        for index, line in enumerate(lines):
           if 'itidep' in line:
              lines[index+1]='            3        %.2e      0.0e-6       %d           1.0e0\n'%(p, nmode)
        f.close()   
        f = open('croco.in','w')
        f.writelines(lines)
    #==============================================================================
    ...
    if restart:
        ...
        restart_files = glob(restart_dir+'jetn_rst*.nc')
        for f in restart_files:
            os.symlink(f, RPATH+'/t0/'+f.split('/')[-1])
