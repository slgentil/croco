...
#  ifdef MASKING
     ...
     &             t(i,j-1,k,nrhs,itrc), t(i,j,k,nrhs,itrc), vel, cdif)
                mask1=rmask(i,j-2)*rmask(i,j+1)
                FEC(i,j)=mask1*flx3+(1-mask1)*flx2
#  else
                FEC(i,j)=flx3
#  endif
              ENDDO
...
#   ifdef UP5_MASKING
                ...
                ENDIF
                FXC(i,j)=mask3*flx5+(1-mask3)*mask1*flx3+
     &                             (1-mask3)*(1-mask1)*mask0*flx2
#   else
                mask1=rmask(i-2,j)*rmask(i+1,j)
                mask2=rmask(i-3,j)*rmask(i+2,j)
                mask0=mask1*mask2
                FXC(i,j)=mask0*flx5+(1-mask0)*mask1*flx3+
     &                         (1-mask0)*(1-mask1)*flx2
#   endif /* UP5_MASKING */
#  else
                FXC(i,j)=flx5
...
#  ifdef MASKING
     ...
     &             t(i-1,j,k,nrhs,itrc), t(i,j,k,nrhs,itrc), vel, cdif)
                mask1=rmask(i-2,j)*rmask(i+1,j)
                FXC(i,j)=mask1*flx3+(1-mask1)*flx2
#  else
                FXC(i,j)=flx3
#  endif
              ENDDO
