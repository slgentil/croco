...
#  ifdef MASKING
     ...
     &             v(i,j  ,k,nrhs), v(i,j+1,k,nrhs), vel, cdif)
                mask1=vmask(i,j-1)*vmask(i,j+2)
                VFeC(i,j)=mask1*flx3+(1-mask1)*flx2
#  else
                VFeC(i,j)=flx3
#  endif
              ENDDO
...
#   ifdef UP5_MASKING
                ...
                ENDIF
                VFxC(i,j)=mask3*flx5+(1-mask3)*mask1*flx3+
     &                             (1-mask3)*(1-mask1)*mask0*flx2
#   else
                mask1=vmask(i-2,j)*vmask(i+1,j)
                mask2=vmask(i-3,j)*vmask(i+2,j)
                mask0=mask1*mask2
                VFxC(i,j)=mask0*flx5+(1-mask0)*mask1*flx3+
     &                         (1-mask0)*(1-mask1)*flx2
#   endif /* UP5_MASKING */
#  else
                VFxC(i,j)=flx5
...
#  ifdef MASKING
     ...
     &             v(i-1,j,k,nrhs), v(i  ,j,k,nrhs), vel, cdif)
                mask1=vmask(i-2,j)*vmask(i+1,j)
                VFxC(i,j)=mask1*flx3+(1-mask1)*flx2
#  else
                VFxC(i,j)=flx3
#  endif
              ENDDO
